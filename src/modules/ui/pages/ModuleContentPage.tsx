...
import { LearningPathService } from "../../../learning-path/LearningPathService";
...
export function ModuleContentPage() {
  ...
  const navItems: NavItem[] = [
    ...
    },
    {
      ...
      icon: (
        <FaChartLine
          className={
            progress.simulationCompleted ? "text-blue-600" : "text-neutral-400"
          }
        />
      ),
      status:
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        ...
        const fetchedContent = await getModuleContentUseCase.execute({
  ...
  const handleResultsComplete = () => {
    ...
    learningPathService.markModuleAsCompleted(parseInt(moduleId || "1", 10));
    learningPathService.markModuleAsInProgress(parseInt(moduleId || "2", 10) + 1);
  };
  ...
  const isNextButtonEnabled = () => {
    ...
    if (currentSection === "results") return progress.resultsViewed;
  };
